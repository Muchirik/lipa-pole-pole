...
class LoanController {
    ...
    async authenticateUser(phoneOrId, pin) {
        // Try to find by phone first
        let userDoc = await db.collection("users").doc(phoneOrId).get();
        if (!userDoc.exists) {
            // Try to find by ID Numbere
            const query = await db.collection("users").where("idNumber", "==", phoneOrId).limit(1).get();
